...
//    final String SEARCH_PAGE = "search.html";
//    final String RESULT_PAGE = "search.jsp";
    /**
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        //1.check valid search values
        try {
            String searchValues = request.getParameter("txtSearchValues");
            if (!searchValues.trim().isEmpty()) {
                //2. call model
                //2.1 new DAO object
                RegistrationDAO dao = new RegistrationDAO();
                //2.2 call method of DAO
                dao.searchLastName(searchValues);
                //3.process result
                List<RegistrationDTO> result = dao.getAccountList();
                url = siteMaps.getProperty(MyApplicationConstants.SearchLastNameFeature.RESULT_PAGE);
                request.setAttribute("SEARCH_RESULT", result);
            }//end search ahs valid value
        } catch (SQLException ex) {
            log("SearchLastNameServlet" + ex.getMessage());
