 ...
 * @author Admin
 */
@WebServlet(name = "PostProductServlet", urlPatterns = {"/postProduct"})
@MultipartConfig
public class PostProductServlet extends HttpServlet {
    ...
    private static final String AWS_OBJECT_KEY_PRE = "https://docabucket.s3.ap-southeast-1.amazonaws.com/";
...
//
            ...
            ProductDTO product = new ProductDTO(3, Integer.parseInt(categoryPost), title, content, objectKey, isFree, Float.parseFloat(inputFee), address, timePosted, true, "pending", null);
            boolean result = dao.createPostProduct(product);
            if (result == true) {

            }
        } catch (SQLException ex) {
            Logger.getLogger(PostProductServlet.class.getName()).log(Level.SEVERE, null, ex);
...
//
//            out.println(categoryPost + "-" + title + "-" + price + "-" + content + "-" + address);
//
//        }
    }
