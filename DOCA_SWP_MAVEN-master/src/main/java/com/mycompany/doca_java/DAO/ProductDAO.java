...
public class ProductDAO {
    ...
    public ProductDTO getProductById(int productId)
     ...
     public boolean canUserPostMoreProducts(int userID) throws SQLException, ClassNotFoundException, NamingException {
        Connection con = null;
        PreparedStatement stm = null;
        ResultSet rs = null;
        ...
        try {
            ...
            if (con != null) {
                // Create SQL query
                String sql = "SELECT COUNT(post_id) AS numPosts FROM post WHERE user_id = ?";
                ...
                if (rs.next()) {
                    int numPosts = rs.getInt("numPosts");
                    return numPosts < 3; // Return true if the user has posted fewer than three products
                }
            }
        } finally {
        ...
        return false; // Return false in case of an error or if the user has already posted three products
    }
