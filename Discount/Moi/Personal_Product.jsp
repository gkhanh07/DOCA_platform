                            ...
                            <div role="tabpanel" class="tab-pane fade ${IN eq 'display' ? 'show active' : ''}" id="display"> 
                                ...
                                <c:forEach items="${listProductOfPersonal}" var="product">
                                    <c:if test="${product.status eq 'approved'}">
                                        ...
                                        <div class="font">
                                            <h5>${product.title}</h5>
                                            <c:if test="${not empty product.listSaled}">
                                                <c:forEach var="sale" items="${product.listSaled}">
                                                    <h6>Giá cũ: <span style="text-decoration: line-through; color: black;" id="productPrice-${product.productId}">${product.price}</span></h6>
                                                    <h4>Giá mới: <span>${sale.new_price}</span>  </h4> <h2 style="color: red">Giảm: ${sale.discount_amount}%</h2>
                                                    <br>
                                                    <h6>Ngày bắt đầu giảm giá: ${sale.start_date} / Ngày kết thúc: ${sale.end_date} </h6>
                                                </c:forEach>
                                            </c:if>
                                            <c:if test="${ empty product.listSaled}">
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <input type="date" class="form-control" name="dateStart" id="discountStartDate-${product.productId}" onchange="updateDiscountStartDate(${product.productId})">
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="discountEndDate-${product.productId}">Ngày kết thúc giảm giá:</label>
                                                            <input type="date" class="form-control" name="dateEnd"  id="discountEndDate-${product.productId}"  onchange="updateDiscountEndDate(${product.productId})">
                                                        </div>
                                        ...
                                        <script>
                                            ...
                                            function updateDiscountEndDate(productId) {
                                                ...
                                                var discountEndDateHiddenInput = document.getElementById("discountEndDate"); // Sửa đổi ID tại đây
