...
public class DiscountDAO {
    ...
    public List<DiscountDTO> listSaled(int productId, int userId) throws SQLException, ClassNotFoundException, NamingException {
        Connection con = null;
        PreparedStatement stm = null;
        ResultSet rs = null;
        List<DiscountDTO> listSale = new ArrayList<>();
        try {
            //1.make connection
            con = DBconnect.makeConnection();
            if (con != null) {
                ...
                while (rs.next()) {
                    DiscountDTO saled = new DiscountDTO(rs.getInt("user_id"), rs.getInt("productId"), rs.getDouble("discount_amount"), rs.getDouble("new_price"), rs.getDate("start_date"), rs.getDate("end_date"), rs.getBoolean("status"));
                    listSale.add(saled);
                }
            }
        } finally {
